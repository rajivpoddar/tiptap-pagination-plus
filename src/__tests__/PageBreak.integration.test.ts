...
describe('PageBreak with PaginationPlus Integration', () => {
  ...
  it('should work with PaginationPlus extension', () => {
    ...
    const pageBreakExt = editor.extensionManager.extensions.find(
      (ext) => ext.name === 'pageBreak'
    );
    const paginationExt = editor.extensionManager.extensions.find(
  ...
  it('should update page break heights dynamically', (done) => {
    ...
    setTimeout(() => {
      const pageBreakElement = container.querySelector('.page-break-node') as HTMLElement;
  ...
  it('should serialize and deserialize correctly with pagination', () => {
    ...
    const hasPageBreak = json.content?.some((node: any) => node.type === 'pageBreak');
  ...
  it('should respect page break force attribute', () => {
    editor.commands.setContent(
      '<div data-page-break="true" data-page-break-force="true"></div>'
    );

    let pageBreakNode: any = null;
    editor.state.doc.descendants((node) => {
      if (node.type.name === 'pageBreak') {
        pageBreakNode = node;
      }
