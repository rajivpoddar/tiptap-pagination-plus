...
describe('PageBreak', () => {
  ...
  it('should have Ctrl+Enter keyboard shortcut defined', () => {
    ...
    const keyboardShortcuts = pageBreakExt?.config.addKeyboardShortcuts;
  ...
  it('should render page break as non-editable div', () => {
    ...
    const renderHTML = pageBreakExt?.config.renderHTML;
  ...
  it('should be included in document serialization', () => {
    ...
    let hasPageBreak = false;
    if (json.content) {
      hasPageBreak = json.content.some((node: any) => node.type === 'pageBreak');
    }
